...
func PutUpstream(id uint, data map[string]interface{}) error {
	upstream := Upstream{}
...
func DeleteUpstream(id uint) error {
	upstream := Upstream{}
	upstream.Model.ID = id

	count := db.Model(&upstream).Association("Sites").Count()
	if count > 0 {
...
func GetUpstream(id uint) (*Upstream, error) {
